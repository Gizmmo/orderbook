...
export function Book({ name, items, isRed = false }: BookProps) {
      ...
      <tbody>
        {items.map((item) => (
          <tr className={styles.bookrow} key={item.price}>
            <td className={cx(styles.bookData, { [styles.bidsPrice]: isRed, [styles.askPrice] : !isRed })}>{item.price.toLocaleString()}</td>
            <td className={styles.bookData}>{item.quantity.toLocaleString()}</td>
